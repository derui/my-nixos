# copy from https://github.com/NixOS/nixpkgs/blob/nixos-unstable/nixos/modules/services/misc/tabby.nix
{
  config,
  lib,
  pkgs,
...
in
{
  ...
  config = lib.mkIf cfg.enable {
    environment = {
      ...
      systemPackages = [
        tabbyPackage
      ] ++ lib.optionals (cfg.acceleration == "vulkan") [ pkgs.vulkan-loader ];
    };
    ...
    systemd =
      ...
      in
      {
        services.tabby = {
          ...
          preStart = ''
            cp -f /etc/tabby/config.toml $TABBY_ROOT/config.toml
          '';
